r"""
模块名称：dlutils.py
实验链路位置：TranAD 依赖的深度学习基础层、Transformer 层、ConvLSTM 和损失函数工具模块。
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        # #25 (2026-05-31) 注: 此处 sin+cos 叠加 (而非 sin/cos 交错) 是 TranAD 论文原实现,
        #   结果: pe[pos,dim] = sin(...) + cos(...) = sqrt(2)·sin(pos·freq + π/4),
        #   与标准 Transformer sin/cos 交错不同。保留原始实现以保持 baseline 公平性。
...
class TransformerEncoderLayer(nn.Module):
    ...
    # ============================================================
    # SCADA实验修改:
    # 名称: PyTorch 2.11 TransformerEncoderLayer 接口兼容
    # 修改原因: torch.nn.TransformerEncoder 在新版调用自定义层时会传入 is_causal,
    #          原 VLDB 代码签名未声明该参数,导致前向计算在进入模型后立即失败。
    # 作用: 接收新版框架传入的 causal 标志,保持原自注意力计算逻辑不变。
    # 数学原理: 本实现仍计算 MultiHeadAttention(src,src,src);
    #          SCADA 窗口使用双向重建注意力,不施加因果 mask。
    # 执行流程: 接收兼容参数 -> 沿用原 self_attn/FFN/残差连接 -> 返回 src。
    # 科研标准: 这是运行时接口兼容修复,不声称改变 TranAD 算法或提升性能。
    # ============================================================
...
class TransformerDecoderLayer(nn.Module):
    ...
    # ============================================================
    # SCADA实验修改:
    # 名称: PyTorch 2.11 TransformerDecoderLayer 接口兼容
    # 修改原因: 新版 TransformerDecoder 可能向自定义 decoder 传入
    #          tgt_is_causal/memory_is_causal,旧签名不能接收。
    # 作用: 接收并忽略仅用于框架调度的因果提示,保留原 TranAD decoder 运算。
    # 数学原理: 仍执行 self-attention + cross-attention + FFN 残差结构,
    #          未添加 causal mask,与原重建式异常检测流程一致。
    # 执行流程: 接收兼容参数 -> 原 self_attn -> 原 cross_attn -> 原 FFN -> 返回。
    # 科研标准: 修复仅解决 PyTorch API 演进导致的崩溃,不改变评价口径。
    # ============================================================
    def forward(
        self,
        tgt,
...
class ComputeLoss:
    ...
    def forward(self, x, x_hat, z, gamma):
        ...
        return loss  # #34 修复: torch.autograd.Variable 自 PyTorch 0.4.0 弃用
...
class Cholesky(torch.autograd.Function):
    def forward(ctx, a):
        # #13 修复 (2026-05-31): torch.cholesky -> torch.linalg.cholesky for PyTorch 2.x compat
        l = torch.linalg.cholesky(a)
        ctx.save_for_backward(l)
        return l
    def backward(ctx, grad_output):
        l, = ctx.saved_tensors  # #35 修复: ctx.saved_variables -> saved_tensors (PyTorch 0.4.0+ 弃用)
        linv = l.inverse()
        inner = torch.tril(torch.mm(l.t(), grad_output)) * torch.tril(
            1.0 - torch.eye(l.size(1), device=l.device).fill_diagonal_(0.5))  # #35 修复: Variable -> torch.eye
